...
pca.data <- t(dataExprs[CSN,]) # get the data for CSN genes from the expression data frame

pca.matrix <- data.matrix(pca.data)

data.pr <- prcomp((pca.matrix), center = TRUE, scale = TRUE) #perform the pca
summary(data.pr)
...
screeplot(data.pr, type = "l", npcs = 11, main = "Screeplot of the 11 PCs")
...
chipnames = rownames(pca.data)

cancer = grep("_ca", chipnames)
healthy = grep("_n", chipnames)

library("factoextra")
fviz_pca_ind(data.pr, geom.ind = "point", pointshape = 21, 
             pointsize = 2, 
          #   fill.ind = pca.data[cancer, healthy,],
             col.ind = "black", 
             palette = "jco", 
             addEllipses = TRUE,
             label = "var",
